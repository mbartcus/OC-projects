...
def plot_words(df, col, height = 15, wieght = 10, max_words=5000, titl='Words'):
    #take not nan values
    df = df[~df[col].isna()]
...
def plot_count_col(df, col, label_col=None, top=None, show_val = False, on_x=True, nrmz = True):
    plt.figure(figsize=(15,8))
    
    if on_x:
        if top==None:
            ax = sns.barplot(y=df[col].value_counts(normalize=nrmz), x=df[col].value_counts(normalize=nrmz).index, data=df);
        else:
            ax = sns.barplot(y=df[col].value_counts(normalize=nrmz)[:top], x=df[col].value_counts(normalize=nrmz).index[:top], data=df);
    else:
        if top==None:
            ax = sns.barplot(x=df[col].value_counts(normalize=nrmz), y=df[col].value_counts(normalize=nrmz).index, data=df);
        else:
            ax = sns.barplot(x=df[col].value_counts(normalize=nrmz)[:top], y=df[col].value_counts(normalize=nrmz).index[:top], data=df);
